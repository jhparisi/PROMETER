import 'MensajesClass.dart';

String mensaje ="";
String fecha = "";
String hora ="";

class MensajesChatClass
{
  final String name;
  String message;
  final String time;
  ...
  MensajesChatClass({this.name, this.message, this.time, this.imageUrl});

  Future obtenerMensaje(List<Mensajes> listMensajes) async{
    listMensajes.forEach((element) {
      print("EL MENSAJE ES: " + element.mensaje);
      mensaje = element.mensaje;
      fecha = element.fecha;
      if(fecha !=""){
        //print("HORA TIENE: " + hora.length.toString()); 
        hora = fecha.substring(5,16);
      }
    });
    messageData = [
      new MensajesChatClass(
        name: "Ezsa Informa",
        message: mensaje.replaceAll("\n", " "),
        time: "Últ. " + hora,
        imageUrl:
...
List<MensajesChatClass> messageData = [
  new MensajesChatClass(
    name: "Ezsa Informa",
    message: mensaje,
    time: hora,
