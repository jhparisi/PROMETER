...
class PTCerrarRuta extends StatefulWidget{
  ...
  PTCerrarRuta({Key key,this.data}): super (key:key);
  _PTCerrarRutaState createState() => _PTCerrarRutaState();
}

class _PTCerrarRutaState extends State<PTCerrarRuta> with WidgetsBindingObserver{ 
                    ...
                    Container(
                      ...
                      child: Align(
                        ...
                        child: PopupMenuButton<InfoButtons>(
                          ...
                          itemBuilder: (BuildContext context) => <PopupMenuEntry<InfoButtons>>[
                            const PopupMenuItem<InfoButtons>(
                              child: Text('Pausar: Tiempo para una parada (comida),\neste tiempo no es computable como jornada de trabajo.',style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold)),
                            ),
                            const PopupMenuItem<InfoButtons>(
                              child: Text('Terminar: Fin de la jornada de trabajo.\nSolo darlo una vez al día,\ny en la finalización de la jornada.',style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold, color: Colors.red)),
                            ),
                            const PopupMenuItem<InfoButtons>(
                              child: Text('Reiniciar: Continua el contador de tiempo \nde trabajo después de la parada realizada',style: TextStyle(fontSize: 12.0, fontWeight: FontWeight.bold,color: Colors.green)),
                            )
                          ],
                        ),
                      )
  ...
  Future<List<TrackingData>> getSendLocalDataToAPI() async{
    ...
    try{
      ...
      if(list != null){
        ...
        if(respuesta=="OK"){
          print("SE HAN REGISTRADO LOS DATOS DE TRACKING DAY, Y SE HAN BORRADO DE LA BD LOCAL");
          _dbprovider.deleteTrackingData(userId);
          //timeSendData=65;
        }
  ...
  void addPlayPause(int status){
    ...
    print("SE HA REGISTRADO EN LOCAL EL PLAYPAUSE CON VALOR="+status.toString());
    ...
    Timer.periodic(Duration(seconds: 1), (timer){
      if(estaPausado==false){
        ...
        if(horaPausa!="0"){
          ...
          Duration trabajadoDif =  new Duration(hours:int.tryParse(splitHT[0]), minutes:int.tryParse(splitHT[1]), seconds:int.tryParse(splitHT[2]));
          print("dif= " +dif.toString());
        ...
        setState(() {
          ...
          if(estaPausado==false){
            ...
            print("Count lastPosition: " + lastPosition.toString() + "-- Count SendData: " + timeSendData.toString());
          }
